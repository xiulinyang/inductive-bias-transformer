...
for n in range(10):
    with (open(f'data_gen/grammar/{GRAMMAR_NAME}/{GRAMMAR_NAME}/{n}.trn', 'w') as train,
         open(f'data_gen/grammar/{GRAMMAR_NAME}_permutation/{GRAMMAR_NAME}_permutation/{n}.trn', 'w') as perm_train,
        open(f'data_gen/grammar/{GRAMMAR_NAME}/{GRAMMAR_NAME}/{n}.dev', 'w') as dev,
        open(f'data_gen/grammar/{GRAMMAR_NAME}_permutation/{GRAMMAR_NAME}_permutation/{n}.dev', 'w') as perm_dev,
        open(f'data_gen/grammar/{GRAMMAR_NAME}/{GRAMMAR_NAME}/{n}.tst', 'w') as test,
        open(f'data_gen/grammar/{GRAMMAR_NAME}_permutation/{GRAMMAR_NAME}_permutation/{n}.tst', 'w') as perm_test,):
        if 'exp3' in GRAMMAR_NAME:
            hh = open(f'data_gen/grammar/{GRAMMAR_NAME}/{GRAMMAR_NAME}/inverse{n}.trn', 'w')
            perm_hh = open(f'data_gen/grammar/{GRAMMAR_NAME}_permutation/{GRAMMAR_NAME}_permutation/inverse{n}.trn', 'w')
        else:
            hh = None
            perm_hh = None
        sents = []
