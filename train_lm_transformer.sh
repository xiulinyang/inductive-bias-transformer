...
fairseq-eval-lm "data-bin/${GRAMMAR}/correct_${SPLIT}-dataset" --path "checkpoints/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt" --tokens-per-sample 512 --gen-subset "test" --output-word-probs --quiet 2> "trans-results/${GRAMMAR}.correct_${SPLIT}.test.txt"
python get_sentence_scores.py -i "trans-results/${GRAMMAR}.correct_${SPLIT}.test.txt" -O "trans_sentence_scores/"

fairseq-eval-lm "data-bin/${GRAMMAR}/incorrect_${SPLIT}-dataset" --path "checkpoints/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt" --tokens-per-sample 512 --gen-subset "test" --output-word-probs --quiet 2> "trans-results/${GRAMMAR}.incorrect_${SPLIT}.test.txt"
